...
export class ObsidianLeafletSettingTab extends PluginSettingTab {
    ...
    createMapSettings(containerEl: HTMLElement) {
        ...
        new Setting(containerEl)
            .setHeading()
            .setName(t("Marker Clustering"))
            .setDesc(t("Enable marker clustering for dense maps"));

        new Setting(containerEl)
            .setName(t("Enable Marker Clustering"))
            .setDesc(
                t(
                    "When enabled, markers that are close together will be clustered into a single marker with a count."
                )
            )
            .addToggle((toggle) => {
                toggle.setValue(this.data.enableMarkerClustering ?? false);
                toggle.onChange(async (v) => {
                    this.data.enableMarkerClustering = v;
                    await this.plugin.saveSettings();
                });
            });

        new Setting(containerEl)
            .setName(t("Max Cluster Radius"))
            .setDesc(
                t(
                    "The maximum radius (in pixels) for a cluster. Lower values create tighter clusters."
                )
            )
            .addText((text) => {
                text.setValue(`${this.data.maxClusterRadius ?? 80}`);
                text.onChange((v) => {
                    const num = parseInt(v);
                    if (!isNaN(num) && num > 0) {
                        this.data.maxClusterRadius = num;
                    }
                });
                text.inputEl.addEventListener("blur", async () => {
                    await this.plugin.saveSettings();
        ...
        new Setting(containerEl)
            .setName(t("Spiderfy on Max Zoom"))
            .setDesc(
                t(
                    "When zoomed in to the maximum level, clusters will spiderfy out."
                )
            )
            .addToggle((toggle) => {
                toggle.setValue(this.data.spiderfyOnMaxZoom ?? true);
                toggle.onChange(async (v) => {
                    this.data.spiderfyOnMaxZoom = v;
                    await this.plugin.saveSettings();
                });
            });

        new Setting(containerEl)
            .setName(t("Zoom to Bounds on Click"))
            .setDesc(
                t(
                    "Clicking a cluster will zoom to show all markers in the cluster."
                )
            )
            .addToggle((toggle) => {
                toggle.setValue(this.data.zoomToBoundsOnClick ?? true);
                toggle.onChange(async (v) => {
                    this.data.zoomToBoundsOnClick = v;
                    await this.plugin.saveSettings();
                });
            });
    }
