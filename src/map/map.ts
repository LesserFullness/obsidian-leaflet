...
export abstract class BaseMap extends Events implements BaseMapDefinition {
    ...
    addFeatures() {
        /** Add GeoJSON to map */
        // 如果启用了聚类功能，使用 markerClusterGroup
        if (this.options.enableMarkerClustering) {
            this.log("Marker clustering enabled, using markerClusterGroup.");
            // 使用 window.L 确保能访问到 Leaflet 全局对象
            const L = window.L || window[LeafletSymbol];
            if (L && L.markerClusterGroup) {
                this.featureLayer = L.markerClusterGroup({
                    maxClusterRadius: this.options.maxClusterRadius || 80,
                    spiderfyOnMaxZoom: this.options.spiderfyOnMaxZoom ?? true,
                    zoomToBoundsOnClick: this.options.zoomToBoundsOnClick ?? true,
                    disableClusteringAtZoom: this.options.maxZoom
                });
            } else {
                this.log("MarkerCluster not found, falling back to featureGroup");
    ...
    updateMarkerIcons() {
        ...
        this.markerIcons.forEach(({ type }) => {
            if (!this.markerIcons.has(type)) {
                this.displaying.set(type, true);
                // 如果启用了聚类，使用 markerClusterGroup，否则使用 layerGroup
                const L = window.L || window[LeafletSymbol];
                if (this.options.enableMarkerClustering && L && L.markerClusterGroup) {
                    this.currentGroup.markers[type] = L.markerClusterGroup({
                        maxClusterRadius: this.options.maxClusterRadius || 80,
                        spiderfyOnMaxZoom: this.options.spiderfyOnMaxZoom ?? true,
                        zoomToBoundsOnClick: this.options.zoomToBoundsOnClick ?? true
                    });
        ...
        [...this.displaying].forEach(([type]) => {
            ...
            if (!this.currentGroup.markers.default) {
                ...
                const L = window.L || window[LeafletSymbol];
                if (this.options.enableMarkerClustering && L && L.markerClusterGroup) {
                    this.currentGroup.markers.default = L.markerClusterGroup({
                        maxClusterRadius: this.options.maxClusterRadius || 80,
                        spiderfyOnMaxZoom: this.options.spiderfyOnMaxZoom ?? true,
                        zoomToBoundsOnClick: this.options.zoomToBoundsOnClick ?? true
                    });
...
export class RealMap extends BaseMap {
    ...
    async buildLayer(layer: {
        ...
        const markerGroups = Object.fromEntries(
            this.markerTypes.map((type) => {
                if (this.options.enableMarkerClustering && L && L.markerClusterGroup) {
                    return [type, L.markerClusterGroup({
                        maxClusterRadius: this.options.maxClusterRadius || 80,
                        spiderfyOnMaxZoom: this.options.spiderfyOnMaxZoom ?? true,
                        zoomToBoundsOnClick: this.options.zoomToBoundsOnClick ?? true
                    })];
                }
                return [type, L.layerGroup()];
            })
        );
        markerGroups.custom = this.options.enableMarkerClustering && L && L.markerClusterGroup ? 
