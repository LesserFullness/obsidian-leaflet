...
export class LeafletRenderer extends MarkdownRenderChild {
    ...
    constructor(
        ...
        this.options = {
            ...
            // 聚类功能参数
            enableMarkerClustering: this.params.enableMarkerClustering !== undefined 
                ? (typeof this.params.enableMarkerClustering === "string" 
                    ? this.params.enableMarkerClustering.toLowerCase() === "true" 
                    : this.params.enableMarkerClustering)
                : this.plugin.data.enableMarkerClustering,
            maxClusterRadius: this.params.maxClusterRadius ?? this.plugin.data.maxClusterRadius,
            spiderfyOnMaxZoom: this.params.spiderfyOnMaxZoom !== undefined
                ? (typeof this.params.spiderfyOnMaxZoom === "string"
                    ? this.params.spiderfyOnMaxZoom.toLowerCase() === "true"
                    : this.params.spiderfyOnMaxZoom)
                : this.plugin.data.spiderfyOnMaxZoom,
            zoomToBoundsOnClick: this.params.zoomToBoundsOnClick !== undefined
                ? (typeof this.params.zoomToBoundsOnClick === "string"
                    ? this.params.zoomToBoundsOnClick.toLowerCase() === "true"
                    : this.params.zoomToBoundsOnClick)
                : this.plugin.data.zoomToBoundsOnClick
        };
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (
                ...
                for (let link of files) {
                    ...
                    if (frontmatter.location || frontmatter.coordinates) {
                        // 优先使用 location，如果 location 有值；否则使用 coordinates
                        ...
                        for (const location of locations) {
                            ...
                            try {
                                lat =
                                    typeof lat === "number"
                                        ? lat
                                        : Number(String(lat).split("%").shift());
                                long =
                                    typeof long === "number"
                                        ? long
                                        : Number(String(long).split("%").shift());
                            } catch (e) {
                                err = true;
                            }
